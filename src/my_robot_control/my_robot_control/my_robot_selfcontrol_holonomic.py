...
class RobotSelfControl(Node):
    ...
    def timer_callback(self):
        ...
        # Publish the robot command (Twist)
    ...
    def laser_callback(self, scan):
        ...
        # Determine zone based on the closest distance angle
        ...
        # React to obstacles based on zone detection
        if closest_distance < self._distanceLimit:
            if zone == "FRONT":
                self._msg.linear.x = -self._forwardSpeed * self._speedFactor  # Move backward
                self._msg.linear.y = 0.0  # Stop lateral movement
                self._msg.angular.z = self._rotationSpeed * self._speedFactor  # Rotate counter-clockwise
            elif zone == "LEFT":
                self._msg.linear.x = 0.0  # Stop forward movement
                self._msg.linear.y = self._forwardSpeed * self._speedFactor  # Move right
                self._msg.angular.z = 0.0
            elif zone == "RIGHT":
                self._msg.linear.x = 0.0  # Stop forward movement
                self._msg.linear.y = -self._forwardSpeed * self._speedFactor  # Move left
                self._msg.angular.z = 0.0
            elif zone == "BACK_LEFT":
                self._msg.linear.x = self._forwardSpeed * self._speedFactor  # Move forward
                ...
                # No obstacle detected or outside FOV
                self._msg.linear.x = self._forwardSpeed * self._speedFactor
                self._msg.linear.y = 0.0
                self._msg.angular.z = 0.0
        else:
